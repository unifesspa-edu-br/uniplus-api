...
- [x] **Externalização Kafka do Selecao RELIGADA** (`feat(selecao)`): wiring extraído para `SelecaoMessagingRegistration.AddSelecaoMessaging` (Schema Registry + `Action<WolverineOptions>` com routing PG queue `domain-events` + tópico Kafka `edital_events`). Standalone e host chamam o mesmo método (paridade, sem drift); o host compõe o configurador na instância única. Selecao 98 (cascading + routing Kafka real) + host boot compõe a PG queue.
...
### P6 — isolamento R8 + OpenAPI por-módulo ✅
- [x] **`CrossModuleReadIsolationTests`**: fato positivo novo — host isento do `ModulesRoster` + afirma que o composition root compõe os 4 módulos internos (referencia as 4 `.API`). Via reflexão (carregar o host no ArchLoader traria 5 `Program` ao grafo). **ArchTests 25/25.**
- [x] **OpenAPI por-módulo no host**: `ModuleApiGroupingConvention` (só no host) atribui `GroupName` por namespace de controller → o `ShouldInclude` default isola cada `/openapi/{modulo}.json`. Sem ela, todo endpoint `GroupName=null` vazava para todos os docs. `OpenApiPorModuloTests` prova ausência de vazamento; baselines standalone **inalterados** (convention não existe fora do host). **Host 17/17.**

### P7 — ops do monólito ✅
- [x] **`docker/Dockerfile.host`** (publish do host), **`docker/init-db.monolito.sql`** (extensões no banco único `uniplus`; schemas vêm das migrations; role única no spike, role-por-schema documentado p/ rollout), **`docker/docker-compose.monolito.yml`** (stack mínima: postgres + redis + monólito; Kafka/MinIO/Keycloak omitidos — não tocados no boot), **`.dockerignore`** (raiz).
- [x] **Smoke end-to-end via Docker** (porta host 28080): `/health/live` 200; `GET /api/{organizacao/unidades, configuracao/campi, selecao/editais}` 200; `/api/campi` sem prefixo → 404; `/openapi/{modulo}.json` isolado (só o prefixo do próprio módulo); 5 schemas (`configuracao, ingresso, organizacao, selecao, wolverine`) criados no banco único pelo boot do container. Build 0/0.
...
- **Co-hosting de `Sdk.Web` colide `appsettings*.json` no publish (NETSDK1152)** — os 4 `.API` referenciados são executáveis Web e cada um copia seu `appsettings*.json` para o output do host, no mesmo destino. No spike resolvido com `ErrorOnDuplicatePublishOutputFiles=false` (configs efetivas = host + env vars). **Rollout:** definir precedência determinística (target que remove os appsettings dos módulos do publish, mantendo só os do host).
- **Faltava `.dockerignore`** — sem ele, `bin/obj` locais entram no contexto de build (mais lento + risco de `project.assets.json`/lock de versão diferente contaminar o restore no container). Adicionado na raiz; beneficia todos os Dockerfiles.
- **Cross-cutting com filtro global + serviço por-DbContext assume "um módulo por processo" (idempotência, ADR-0027)** — achado P1 da revisão Codex: `AddIdempotency<TDbContext>` usava `PostConfigure<MvcOptions>` acumulativo (N módulos ⇒ N filtros globais ⇒ 409 espúrio) + `IIdempotencyStore` não-keyed ("último vence", schema errado). **Corrigido** (`fix(idempotency)`): filtro genérico `IdempotencyFilter<TDbContext>` aplicado por controller do módulo via `IApplicationModelConvention`, assinatura `AddIdempotency<TDbContext, TApiMarker>`, store concreto, `TryAddEnumerable`; fitness test `IdempotenciaCoHostingTests`. **Padrão a auditar no rollout:** qualquer outro cross-cutting que registre filtro global MVC ou serviço scoped único por módulo (revisar antes do merge).
## Estado atual
**P1–P7 ✅ — spike completo.** Fundação: schema param (`f39360b`) → IUnitOfWork por módulo (`a140128`) → schema-por-módulo nos 4 (`18daddd`,`b736c14`,`c841fea`,`2da84b4`) → `Add{Modulo}Module` nos 4 (`e5c79b7` + `13...`) → **host composition root** (`feat(host)`) compondo os 4 módulos + Wolverine numa instância + 5 connections→`uniplus` → **prova de runtime** (`tests/Unifesspa.UniPlus.Host.IntegrationTests`): boot do processo único, 5 schemas, leitura in-process → **P1** prefixo de rota por módulo + colisão → **P6** fato positivo R8 + OpenAPI por-módulo isolado → **P7** Dockerfile + compose único + init-db variante, **smoke end-to-end via Docker**. **Build 0/0; host 17/17; Configuracao 62 + Organizacao 47 + ArchTests 25/25.**

A **prova do spike está completa**: monólito modular boota num processo único (validado por Testcontainers E por `docker compose`), banco único schema-por-módulo, consumidor cross-módulo lê a Unidade viva in-process via `IUnidadeReader` (desbloqueio #588) com fronteira R8 preservada, roteamento co-hospedado sem colisão (prefixo `api/{modulo}/`) e documentos OpenAPI isolados por módulo.

Kafka do Selecao **religado**. Falta apenas **escrever o ADR**.

## Como retomar — próximo: ADR
Branch `spike/monolito-modular`. Todas as fases (P1–P7) provadas em runtime; Kafka do Selecao religado. Restante:
1. **Escrever o ADR** do monólito modular purista (decisões em "Decisões para o ADR"; achados em "Achados") e remover este checkpoint no rollout.
2. **Follow-up frontend (rollout):** prefixo de rota é breaking change — re-sincronizar contrato no `uniplus-web` e regenerar client após o merge.
3. **Follow-ups de rollout listados nos achados:** role-por-schema (isolamento), precedência determinística de `appsettings` no co-hosting, regenerar `packages.lock.json`, auditar outros cross-cutting que registrem filtro global MVC.

Gate final do spike: `dotnet build` + `dotnet test` completo verde → **escrever o ADR** e fechar.
