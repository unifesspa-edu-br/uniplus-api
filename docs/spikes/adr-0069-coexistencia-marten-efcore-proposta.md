...
11. ✅ **escala horizontal — concorrência otimista por stream**, em dois níveis:
    ...
    - ⟳ **Não exercitado:** a mesma escala sob a topologia **co-hospedada** (EF main + Marten ancillary, `ConfiguracaoCoexistencia`). A garantia de concorrência é a mesma (mecanismo por-stream do Marten, independe de primário/ancillary), mas a corrida `[WriteAggregate]` multi-réplica naquela topologia específica não foi rodada — o handler ancillary `[MartenStore]` esbarra no artefato de code-gen multi-app no mesmo processo de teste (não-issue em produção, 1 app/processo).
