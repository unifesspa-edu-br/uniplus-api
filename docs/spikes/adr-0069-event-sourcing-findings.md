...
- `CoexistenciaBootTests` (boot dos dois stores), `CoexistenciaTests` (handler EF main atômico+outbox; event store ancillary utilizável), `MultiNoLeaderElectionTests` (2 réplicas em `Balanced`, eleição de líder, failover com ejeção do nó morto e continuidade) `ConcorrenciaStreamTests` (mecanismo: dois writers no mesmo stream → concorrência otimista por versão imposta no banco; perdedor leva `EventStreamUnexpectedMaxEventIdException`, retry, sem lost update) e `EscalaDuasReplicasTests` (ponta a ponta no host ES: **2 réplicas `Balanced`** retificam o mesmo stream concorrentemente; handler `[WriteAggregate]` + retry automático do Wolverine convergem todas, sem lost update) provam a coabitação, o cluster e a escala horizontal. A escala sob a topologia co-hospedada específica (EF main + Marten ancillary) herda o mesmo mecanismo de concorrência, mas não foi rodada como teste (artefato de code-gen multi-app no processo de teste).
