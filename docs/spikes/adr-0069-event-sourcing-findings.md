...
- A limitação "um message store por host" é **pré-Wolverine 5**; o projeto roda 5.39, que suporta múltiplos stores via **um 'main' + N 'ancillary'** (`ancillary` = store secundário/auxiliar; termo do Wolverine). A config ingênua (dois 'main') falha com erro acionável (`InvalidWolverineStorageConfigurationException`).
...
- `CoexistenciaBootTests` (boot dos dois stores), `CoexistenciaTests` (handler EF main atômico+outbox; event store ancillary utilizável), `MultiNoLeaderElectionTests` (2 réplicas em `Balanced`, eleição de líder, failover com ejeção do nó morto e continuidade) `ConcorrenciaStreamTests` (mecanismo: dois writers no mesmo stream → concorrência otimista por versão imposta no banco; perdedor leva `EventStreamUnexpectedMaxEventIdException`, retry, sem lost update), `EscalaDuasReplicasTests` (ponta a ponta no host ES: **2 réplicas `Balanced`** retificam o mesmo stream; handler `[WriteAggregate]` + retry automático do Wolverine convergem todas, sem lost update) e `EscalaCoHospedadaProcessosTests` (ponta a ponta na topologia co-hospedada com **2 processos/portas reais**: EF main + Marten ancillary, retificações concorrentes via HTTP no mesmo stream → convergem sem lost update) provam a coabitação, o cluster e a escala horizontal — inclusive sob a topologia co-hospedada exata.
- **Achado sobre o handler ancillary:** num host em que o Marten é **apenas ancillary** (sem `AddMarten` primário), a injeção de `IDocumentSession` via `[MartenStore]` **não resolve** o tipo ambiente. Padrão robusto adotado: o handler injeta o **store marcador** (`IEditalEsStore`) e abre a sessão explicitamente — usado tanto pelo append direto in-process quanto pelo host co-hospedado (`EscalaCoHospedadaProcessosTests`), que prova a topologia exata com **2 processos reais**.
