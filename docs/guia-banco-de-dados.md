...
- **Remoção exige conferir referência viva**, além das colunas de auditoria: usar uma linha não atualiza a linha usada, e as referências entre cadastros deste módulo são texto sem chave estrangeira (`TipoDocumento.categoria`, `PrecedenciaFase.antecessora_codigo`). O banco não recusa o `DELETE` — deixa a referência pendurada. A migration confere com um `NOT EXISTS` sobre quem aponta para o código; havendo referência, a remoção é ato administrativo.
