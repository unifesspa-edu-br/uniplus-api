...
- `btree_gist` — pré-requisito das exclusion constraints GIST sobre intervalos temporais ([ADR-0060](adrs/0060-junction-tables-por-entidade-com-view-unificada.md)). Provisionada em dev pelo `init-db.sql` e, nos demais ambientes, pela migration do `DbContext` que a exige (idempotente, `CREATE EXTENSION IF NOT EXISTS`) — os containers efêmeros dos testes de integração não executam o `init-db`, então a migration é a única fonte. A aplicação original (junction de áreas de interesse) foi removida no KILL do eixo de Área (Epic #600); a aplicação viva hoje é a constraint `ex_tipo_ato_publicado_codigo_vigencia`, no schema `publicacoes`, que impede duas versões vivas do mesmo tipo de ato de valerem no mesmo dia. Como a constraint vive em SQL cru e não entra no `ModelSnapshot`, um squash de migrations a descartaria em silêncio: `TipoAtoPublicadoPersistenceTests` consulta o `pg_constraint` para impedi-lo.
...
# uniplus_configuracao, uniplus_organizacao (usuários _app dedicados)
```
...
- Escolhido o seed em migration num catálogo **administrável**: **`UpdateData` e `DeleteData` gerados não entram**. O primeiro sobrescreveria a edição do operador; o segundo apagaria a linha fisicamente, sem o soft-delete auditado da API. A migration só acrescenta (`INSERT … ON CONFLICT DO NOTHING`).
- **Item semeado que nasceu errado** se corrige por **nova migration** — nunca editando a já aplicada —, com o comando condicionado a `updated_at IS NULL AND created_by IS NULL`. Assim alcança a linha que ninguém tocou, em base nova e antiga igualmente, e não alcança a que o operador editou. **Isso vale para atributo, não para o código**: o código é a identidade que outras tabelas guardam como texto, então renomeá-lo por `UPDATE` deixa referência pendurada igual ao `DELETE` — exige a mesma checagem do item seguinte. Corrigir só pela tela consertaria o ambiente onde alguém rodou a correção e deixaria o catálogo dependendo da idade do banco, já que esses arquivos saíram do Newman. Onde a linha tiver dono, aí sim a correção é ato administrativo.
- **Remoção exige conferir referência viva**, além das colunas de auditoria: usar uma linha não atualiza a linha usada, e as referências entre cadastros deste módulo são texto sem chave estrangeira (`TipoDocumento.categoria`, `PrecedenciaFase.antecessora_codigo`). O banco não recusa o `DELETE` — deixa a referência pendurada. A migration confere com um `NOT EXISTS` sobre quem aponta para o código. Se quem aponta for **também linha semeada e intocada**, a remoção segue por migration, em cascata na ordem certa; se tiver dono, aí sim é ato administrativo.
- Catálogo **sem CRUD** (como os de fato, `FatoCandidato` e `FatoValorDominio`) é o caso inverso: a migration é o único caminho de escrita que existe, então alterar por ela é legítimo — não há operador cuja edição possa ser sobrescrita, e o blame fica no PR. O discriminador é sempre o mesmo: **se a tela permite editar aquele campo, a migration não pode tocá-lo**.
- **Ausência de CRUD não basta:** havendo regime de congelamento, ele vem antes. `RegraCatalogo` é seed-governado e somente-leitura, mas a [ADR-0112](adrs/0112-fronteira-append-only-do-catalogo-de-regras.md) só admite substituir enquanto nenhuma `VersaoConfiguracao` referenciar a entrada. O caminho de escrita responde *quem* altera; o congelamento responde *se* ainda pode.
