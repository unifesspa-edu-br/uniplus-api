...
- Escolhido o seed em migration num catálogo **administrável**: **`UpdateData` e `DeleteData` gerados não entram**. O primeiro sobrescreveria a edição do operador; o segundo apagaria a linha fisicamente, sem o soft-delete auditado da API. A migration só acrescenta (`INSERT … ON CONFLICT DO NOTHING`); alterar ou remover é ato administrativo, pela tela.
