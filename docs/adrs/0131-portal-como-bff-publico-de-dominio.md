...
A leitura é anônima e **pode ser alcançada diretamente** — é um Open Host Service, e o seu propósito é servir consumidores, não apenas um.

Justamente por isso, **o critério de visibilidade vive no contrato, e não no Portal**. Um gate aplicado apenas no backend do portal seria contornado por quem consultasse Seleção diretamente, que é o uso que esta decisão autoriza. Quem responde é quem confere. Isso alinha o certame publicado com o que o sistema já pratica: o registro de atos é anônimo por decisão da [ADR-0105](0105-modulo-publicacoes-registro-central-dos-atos.md), os catálogos de configuração são anônimos, e a renderização do formulário de inscrição já é pública. A proteção vem do contrato — tipos declarados e ausência de oráculo de existência —, não de obscuridade de rede.
...
- **O contrato público tem versão própria, e ela entra na chave de cache e no identificador de conteúdo.** O hash da configuração congelada identifica o envelope, não a projeção: acrescentar um campo ao contrato público não muda aquele hash, e um cache endereçado só por ele continuaria servindo a resposta antiga depois do deploy. Chave e identificador de conteúdo compõem **versão da projeção com hash da configuração**.
  ...
  **Mas o selo sozinho não cumpre a garantia de que uma retificação não fica encoberta.** Ele é conhecido pela origem, e o endereço da página não muda quando o certame é retificado — de modo que uma representação já guardada na borda ou no navegador seria servida sem que ninguém consultasse a origem. Por isso **as respostas públicas são emitidas com revalidação obrigatória** — o cliente pode guardar, mas precisa confirmar antes de usar, e é na confirmação que o selo faz o seu trabalho.

A regra vale para o detalhe **e para a vitrine**, pelo mesmo motivo e com o mesmo risco: uma publicação ou retificação insere ou reposiciona um certame numa página da listagem sem que a URL daquela página mude, de modo que uma representação guardada continuaria omitindo o que já vigora. Limitar a revalidação ao detalhe deixaria a porta de entrada do portal servindo lista vencida.

  **E o selo da coleção não é o selo de um certame.** Uma página reúne vários, e muda quando uma publicação a insere ou quando uma retificação a reposiciona — sem que a configuração de nenhum dos certames já listados mude. Um identificador definido como versão da projeção mais o hash de uma configuração faria a revalidação responder "não mudou" sobre a lista vencida, que é exatamente o que a revalidação foi introduzida para impedir. O identificador de uma página cobre **quem a compõe, em que ordem e em que versão**; não havendo como compô-lo, a coleção não é guardada.
...
- **O cursor de paginação não trafega entre fronteiras, e a chave que o cifra não é compartilhada.** O cursor é opaco, cifrado e carrega o recurso a que pertence justamente para impedir reuso entre coleções ([ADR-0026](0026-paginacao-cursor-opaco-cifrado.md)). Compartilhar a chave com o Portal lhe daria a capacidade de cunhar cursor para qualquer coleção de Seleção.
  ...
  Resta uma assimetria a tratar: o cursor da origem carrega o próprio prazo, cifrado com chave que o Portal deliberadamente não tem — logo ele não pode lê-lo. Se o prazo da origem for o menor dos dois, o cursor externo continuaria válido depois de a origem já recusar o interno. **A validade emitida pelo Portal nunca ultrapassa a da origem**: o prazo trafega ao lado do token, autenticado, ou é limitado por construção à política conhecida da origem.

  E há um efeito que decorre de ordenar por prazo: **o prazo muda**. Uma retificação que altere o encerramento de um certame durante a navegação reposiciona o item em relação à âncora que o cursor guarda — e quem cruza a âncora num sentido aparece duas vezes, quem cruza no outro desaparece. Encapsular o cursor da origem preserva a âncora, não a coleção. A navegação fica **presa à revisão da coleção em que começou**; quando ela avança, o cliente é avisado e recomeça, em vez de receber uma lista silenciosamente inconsistente.

- **A indisponibilidade das origens tem respostas distintas.** Ausência do registro do ato **degrada** enquanto o certame já é visível — o que, pelo critério de visibilidade adiante, significa atos posteriores ainda não drenados, e não o ato criador: a linha do tempo vem vazia ou incompleta, com aviso neutro, e essa resposta não é cacheada — como também não o é a resposta em que o documento do edital ainda está sendo copiado para o acervo ([ADR-0132](0132-armazenamento-publico-separado-para-documento-publicado.md)). A regra vale para todo estado transitório: o selo do cache compõe versão da projeção e hash da configuração, e nenhum dos dois muda quando a pendência se resolve. Ausência do contrato de Seleção **não** degrada: o Portal não serve conteúdo vencido, porque a configuração publicada tem efeito jurídico e uma retificação não pode ser encoberta por cache; responde indisponibilidade temporária, sinalizando quando reencaminhar.
- **A prontidão do Portal cobre o que o processo serve — não menos, e não mais.** Ele agrega hoje, sob a mesma sonda, verificações de banco, cache, armazenamento de objetos, mensageria e provedor de identidade. Sair do ar por causa de mensageria ou armazenamento, que nenhuma rota servida usa, é falso negativo — e derrubaria a vitrine anônima por dependência que ela não toca.

  Mas o critério é **o processo**, e não a vitrine: ele também expõe as rotas compartilhadas de sessão e de perfil, autenticadas. Enquanto elas coabitarem o mesmo deployable, a verificação do provedor de identidade **permanece** — marcar o processo pronto com o provedor fora manteria tráfego chegando a rotas que não têm como validar credencial. Separar a superfície anônima em processo próprio mudaria essa conta; enquanto não muda, prontidão honesta inclui o que as rotas autenticadas precisam.

### O que torna um processo visível

Um processo aparece no portal quando tem versão de configuração vigente **e o ato que a criou está registrado**. A versão carrega, congelados, o identificador e o hash desse ato, referenciados por valor conforme [ADR-0061](0061-referencia-cross-modulo-via-snapshot-copy.md) — mas carregar a referência não é o mesmo que o ato existir.

A distinção não é teórica. O registro do ato acontece por mensagem durável depois da publicação, e uma recusa de mérito — tipo sem versão vigente, vaga de numeração já ocupada — **só aflora depois** do sucesso da publicação. Nesse caso a mensagem vai direto para a fila morta, sem retentativa, e a [ADR-0108](0108-registro-do-ato-por-mensagem-duravel.md) é explícita quanto ao que sobra: *"registro recusado não reserva a vaga do certame, e a versão de configuração permanece publicada"*. Existe, portanto, um estado durável em que há versão vigente e nenhum ato — e tratá-lo como equivalência faria o portal divulgar um certame sem ato normativo correspondente, que é precisamente o que a publicidade do ato deveria garantir.
...
- **Prontidão honesta:** a sonda não reprova por dependência que nenhuma rota servida usa, e não aprova o processo quando uma rota que ele expõe está inoperante — incluindo as autenticadas, enquanto compartilharem o deployable.
