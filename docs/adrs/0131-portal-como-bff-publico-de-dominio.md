...
- **A prontidão do Portal responde por quem ele serve, e o critério é esse — não uma lista.** Uma sonda que reprova por dependência sem consumidor tira do ar um processo capaz de atender; uma que aprova com rota inoperante manda tráfego para o erro. Ambos são desonestos, e a decisão fixa o critério para distinguir:
  ...
  - **não** entra o que só é exercido no arranque, nem o que nenhuma rota toca;
  - **não** entra origem consultada por rede. A indisponibilidade de Seleção ou de Publicações é tratada na resposta, com a distinção da cláusula anterior, e não retirando o processo de serviço: pô-la na sonda propagaria em cascata uma falha que já tem tratamento próprio, e derrubaria também as rotas que não dependem dela.

  A lista concreta de verificações é da implementação, que conhece o que cada rota consome em cada momento. À decisão cabe o critério — e a consequência de que ele seja revisto sempre que o processo ganhar ou perder rota.
...
- **Navegação estável:** percorrer a vitrine devolve cada certame uma vez, sem repetição nem omissão, mesmo que uma retificação altere prazos durante o percurso; e um endereço de continuação nunca sobrevive ao da origem que ele encapsula.
