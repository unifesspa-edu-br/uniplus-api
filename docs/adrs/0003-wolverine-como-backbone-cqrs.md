...
>
> **Emenda (2026-06-26):** a validação FluentValidation do pipeline CQRS passou a usar o pacote oficial `WolverineFx.FluentValidation` (`opts.UseFluentValidation(RegistrationBehavior.ExplicitRegistration)`), substituindo o `WolverineValidationMiddleware` custom que resolvia `IValidator<>` via `IServiceProvider` (service location). O middleware gerado pelo pacote injeta os validators tipados pelo codegen — eliminando o warning de service-location e sendo forward-compat com a `ServiceLocationPolicy.NotAllowed` que vira default no Wolverine 6.0. O contrato externo não muda: a mesma `FluentValidation.ValidationException` é lançada e mapeada como 422 pelo `GlobalExceptionMiddleware`, e os validators continuam descobertos pelos `AddValidatorsFromAssembly` de cada módulo (por isso `ExplicitRegistration`). Para preservar o masking de PII (LGPD), um `IFailureAction<>` próprio (`PiiSafeValidationFailureAction`) substitui o default do pacote — que interpolaria o `ToString()` do command (com CPF/CNPJ/nome) na mensagem da exceção logada; o nosso lança `new ValidationException(failures)`, mantendo a mensagem restrita às falhas de regra, como o middleware antigo fazia. `WolverineFx.FluentValidation` é parte do próprio Wolverine (JasperFx), não do ecossistema CritterStack — permitido pelos riscos desta ADR. O `WolverineLoggingMiddleware` permanece custom.
>
> **Emenda (2026-08-04):** a proibição de importar `Wolverine.*` em Application/Domain ganha uma exceção estreita para `Wolverine.Attributes` — o namespace onde vive `[NonTransactional]`. Handlers que injetam um reader cross-módulo cujo `DbContext` difere do `DbContext` da própria `UnitOfWork` do módulo (ex.: `IUnidadeReader`, de Organização Institucional, injetado em `CriarProcessoSeletivoCommandHandler` de Seleção, issue #849) fazem o detector de transação do Wolverine.EntityFrameworkCore (`AutoApplyTransactions`, ADR-0004) falhar no boot por ambiguidade — dois `DbContext` alcançáveis, um só lugar para enrolar. `[NonTransactional]` é o único mecanismo que o próprio Wolverine expõe para essa exceção; não existe alternativa fluente registrável a partir do composition root (conferido por reflection sobre `Wolverine.dll` 5.40.0 — nenhuma API de exclusão de transação por tipo de mensagem fora do atributo). `Wolverine.Attributes.NonTransactionalAttribute` deriva só de `System.Attribute` (confirmado por reflection: zero campos, zero acoplamento ao `IMessageBus`/dispatch) — é um marcador lido pelo codegen em tempo de build, não uma dependência de runtime do framework. A exceção cobre **só** o namespace `Wolverine.Attributes`; todo o resto de `Wolverine.*` (bus, runtime, EF Core middleware) continua proibido em Application/Domain pela mesma regra de sempre. `CriarOfertaCursoCommandHandler` (Configuração) já usa `[NonTransactional]` pelo mesmo motivo — Configuração não tem o fitness test R2 (`Unifesspa.UniPlus.Configuracao.ArchTests` não existe), então essa emenda também fecha a lacuna de cobertura entre os dois módulos, sem afrouxar o restante da regra.
