...
# ADR-0056: Módulo Configuração e desmembramento read-side cross-módulo via IXxxReader
...
| `TipoEtapa` | Cadastro administrativo de tipos de etapa de processo seletivo | Configuracao + Selecao |
...
A justificativa para um módulo `Configuracao` separado repousa em:
...
[ADR-0001](0001-monolito-modular-como-estilo-arquitetural.md) (monolito modular, cross-módulo via Kafka apenas) governa **mutações write-side cross bounded contexts** — eventos, mudanças de estado, dispatch de comandos. Para **consultas read-side de dados de referência** (renderizar dropdowns, validar codes), a opção de desmembramento: in-process DI síncrono é permitido via interfaces `IXxxReader` publicadas em assemblies `.Contracts`, enforçado por fitness test.
...
- **D**: REST admin genérica `/api/configuracao/{tipo}` com payload polimórfico, single resource.
- **E**: Módulo `Configuracao` dedicado para catálogos cross-cutting; desmembramento read-side via `IXxxReader` em `.Contracts`; write-side permanece restrita a Kafka.

## Resultado da decisão

**Escolhida:** "E — módulo `Configuracao` dedicado com desmembramento read-side via `IXxxReader`", porque é a única opção que respeita a disciplina do monolito modular (ADR-0001) com fitness tests enforcando o pattern, mantém latência do wizard em níveis aceitáveis (sub-millisecond warm) e preserva per-resource vendor MIME (ADR-0028).
...
2. **`TipoEtapa`** → Configuracao, por emenda da ADR-0123; Seleção o consome por `ITipoEtapaReader` e congela snapshot-copy em `EtapaProcesso`, sem FK cross-schema.
...
### Desmembramento cross-módulo read-side (refina ADR-0001)
...
4. A implementação é registrada como Singleton respaldada por **distributed cache (Redis 8 via `IDistributedCache`, TTL 5 minutos)**. O cache armazena o **raw set por recurso** sob chave canônica (`configuracao:{recurso}`, ex.: `configuracao:modalidades`), não views per-caller; o filtro de áreas acontece após retrieval do cache, no reader, baseado no parâmetro `areasCaller` explícito. Cache L1 in-process (`IMemoryCache`) **NÃO** é adicionado em V1 — é otimização adiada gated por evidência de P95 latency ou throughput (ver Riscos).
5. Invalidação via subscriber de evento de domínio Kafka (`ModalidadeAtualizadaEvent` triggera `IDistributedCache.RemoveAsync("configuracao:modalidades")`). O `DEL` único é atômico cross-replicas — não há janela de inconsistência cross-pod.
...
7. A entidade carrega atributo `[ReferenceData]` (declarado em `Configuracao.Contracts`).
...
1. Um segundo módulo (Ingresso, Portal, Auxilio Estudantil) assina como consumer Kafka de um evento de catálogo — tipicamente porque mantém projeção própria ou invalida cache local que o processo Configuracao não alcança via Redis.
...
- `Configuracao.Contracts` pode depender de `OrganizacaoInstitucional.Contracts` (para `AreaCodigo`).
- `LocalProva` em `Selecao.Domain` referencia `EnderecoOrigemId: Guid?` — sem dependência de domain em `Configuracao.Domain.Endereco`; validação da FK é por value-copy via `IEnderecoReader.ObterPorIdAsync` no command handler per [ADR-0061](0061-referencia-cross-modulo-via-snapshot-copy.md).

### Contrato REST por catálogo

Cada catálogo em Configuracao é seu próprio recurso REST com vendor MIME (estende [ADR-0028](0028-versionamento-per-resource-content-negotiation.md)):
...
- `GET /api/configuracao/{recurso}` — leitura; filtrada pelas áreas do caller; sem cursor pagination (catálogos são reference data bounded ≤ 100 linhas; exceção deliberada a [ADR-0026](0026-paginacao-cursor-opaco-cifrado.md)).
- `GET /api/configuracao/{recurso}/{id:guid}` — single resource com HATEOAS `_links` ([ADR-0029](0029-hateoas-level-1-links.md)).
- `POST /api/admin/configuracao/{recurso}` — admin área-scoped per [ADR-0057](0057-areas-rbac-snapshot-historia-invariantes.md); `Idempotency-Key` required.
- `PUT /api/admin/configuracao/{recurso}/{id:guid}` — admin área-scoped; idempotente.
- `DELETE /api/admin/configuracao/{recurso}/{id:guid}` — soft delete apenas.
...
- ADR-0001 refinada com desmembramento preciso fitness-test-enforced — não violada ad hoc.
...
- Adicionar novo consumer module é trivial: referencia `Configuracao.Contracts`, injeta `IModalidadeReader`. Sem nova infra.
...
- Adiciona 2 novos módulos à solution (`Configuracao` + `OrganizacaoInstitucional`).
...
- Reader é dependente de disponibilidade de Redis; outage degrada todos reference-data reads para PostgreSQL hits diretos. Mitigado por Redis ser dependência stack-wide já (sessions, idempotency cache) — failure mode é compartilhado, não específico a este desmembramento.
...
- Revisão arquitetural de PR: PRs que adicionem catálogo em módulo errado (ex.: Modalidade fora de Configuracao) são rejeitados.
...
- **Contras**: Quando Ingresso precisa de `Modalidade` (matriculation eligibility check, near-certainty em 6-12 meses dado roadmap), ou (i) Ingresso importa reader do Selecao (aceitável per desmembramento, mas signal coupling) ou (ii) constrói duplicate local de `Modalidade` (drift inevitável). Futuros módulos compostam o problema. `Modalidade` e `NecessidadeEspecial` são explicitamente federal-universal — colocá-las em Selecao misrepresenta seu escopo.
...
### D — REST admin genérica `/api/configuracao/{tipo}` polimórfica
...
### E — Módulo dedicado com desmembramento (escolhida)
