...
O atributo `cpf` é exposto pelo client scope `uniplus-profile` (existente no realm) como claim no `id_token` e no `/userinfo` — consumido pelo frontend e pelo `uniplus-api` sem chamada extra. Em logs estruturados, o valor é mascarado como `***.999.999-**` pelo `PiiMaskingEnricher` (ADR-0011).
