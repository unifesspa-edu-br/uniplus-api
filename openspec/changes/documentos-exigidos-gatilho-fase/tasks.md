...
- [x] 7.2 Smoke via Newman (não `/smoke-crud` — o recurso é um agregado com 9 dimensões e fluxo de publicação, fora do template de CRUD flat da skill): coleção própria `src/selecao/Unifesspa.UniPlus.Selecao.API/postman/` cobre TODOS os 22 endpoints de `ProcessoSeletivoController` + 2 de `DocumentosEditalController` contra o stack docker completo, incluindo `documentos-exigidos` com gatilho DNF condicional e o ciclo completo de publicação/retificação/sessão editorial. **56/56 requests, 58/58 assertions** passam de ponta a ponta (49/49 na primeira rodada + 7 requests/8 assertions do cenário-alvo e2e da 7.3). No caminho, achado e corrigido um bug real de infraestrutura pré-existente e não relacionado a Seleção — dois `IMinioClient` (interno/público) no mesmo processo com resolução de DI ambígua quebravam o upload de documento do Edital (issue #904, PR #906, mergeado antes desta task); ver `postman/README.md` §Achados para o diagnóstico completo.
- [x] 7.3 Cenário-alvo end-to-end: nível de ensino (GERAL, sem gatilho), renda (`MODALIDADE EM` cotas LB_PPI/LB_Q), laudo (`CONDICAO_ATENDIMENTO IGUAL PCD`), reservista (`SEXO IGUAL MASCULINO` **E** `FAIXA_ETARIA MAIOR_IGUAL 18`, mesma cláusula DNF); publicar e conferir o bloco congelado (V1.2, `exigencias[]`) — executado via a mesma coleção Newman da 7.2, que agora inclui os 4 sub-casos de gatilho DNF + 4 testes de borda (valor fora do domínio, decimal onde exige inteiro, operador incompatível com domínio numérico, GERAL com condição viva/CA-01), todos com asserção no `code` do `ProblemDetails` (não só no status 422). `Obter Snapshot Vigente` confere as 5 exigências (GERAL + 4 CONDICIONAL) e seus gatilhos byte-a-byte no bloco congelado. Simplificação assumida: todos os itens usam a mesma fase de cronograma (a única fase `AVALIACAO` que o smoke provisiona) em vez de fases distintas por sub-caso — o objetivo da task é a resolução correta do gatilho DNF/congelamento, não a segregação por fase.
