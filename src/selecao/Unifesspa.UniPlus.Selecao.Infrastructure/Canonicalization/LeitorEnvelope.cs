...
internal sealed class LeitorEnvelope
{
    ...
    public DateTimeOffset Instante(JsonObject pai, string chave, string path)
    {
        ...
        // Texto exige a chave presente e não-nula; o formato canônico é conferido uma vez só,
        // em InstanteOpcional — duas cópias divergiriam na próxima mudança de formato.
        _ = Texto(pai, chave, path);

        return Falhou ? default : InstanteOpcional(pai, chave, path) ?? default;
    }
