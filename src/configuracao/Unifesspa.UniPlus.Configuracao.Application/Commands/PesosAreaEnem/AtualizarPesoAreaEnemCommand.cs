...
/// são imutáveis — mudá-los caracterizaria outra linha, não uma edição — e não
/// constam no payload.
/// </summary>
/// <remarks>
...
/// <c>0m</c> quando o campo é omitido, um peso zero é válido no domínio (não é
/// distinguível de "campo ausente"), e a omissão sobrescreveria silenciosamente o
/// valor configurado (ex.: um corte 450 vira 0 ao editar só a base legal). Com
/// <c>[JsonRequired]</c> a omissão vira 400 na desserialização. O cliente sempre
/// reenvia o estado completo da linha. <c>BaseLegal</c> é <c>string?</c> sem
/// default (ADR-0125): nulo para o campo ausente chegar à validação de domínio
/// em vez do <c>[ApiController]</c>; sem default, para permanecer <c>required</c>
/// no schema OpenAPI.
/// </remarks>
