...
using Unifesspa.UniPlus.Configuracao.Application.Queries.TiposProcesso;
...
/// <summary>Leitura pública de itens ativos e manutenção por plataforma-admin (UNI-REQ-0098).</summary>
[ApiController]
...
public sealed class TiposProcessoController : ControllerBase
{
    ...
    private readonly IResourceLinksBuilder<TipoProcessoDto> _linksBuilder;

    public TiposProcessoController(ICommandBus commandBus, IQueryBus queryBus, IDomainErrorMapper mapper, IResourceLinksBuilder<TipoProcessoDto> linksBuilder)
    {
    ...
    public async Task<IActionResult> Listar([FromCursor(ResourceTag)] PageRequest page, CancellationToken cancellationToken)
    {
        ...
        ListarTiposProcessoResult resultado = await _queryBus.Send(
            new ListarTiposProcessoQuery(page.AfterId, page.Limit, page.Direction), cancellationToken).ConfigureAwait(false);
    ...
    [HttpGet("tipos-processo/{id:guid}")]
    [AllowAnonymous]
    [VendorMediaType(Resource = "tipo-processo", Versions = [1])]
    ...
    {
        TipoProcessoDto? tipo = await _queryBus.Send(new ObterTipoProcessoPorIdQuery(id), cancellationToken).ConfigureAwait(false);
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status422UnprocessableEntity)]
    public async Task<IActionResult> Criar([FromBody] CriarTipoProcessoCommand command, CancellationToken cancellationToken)
    {
    ...
    [HttpPut("admin/tipos-processo/{id:guid}")]
    [Authorize(Roles = "plataforma-admin")]
    [RequiresIdempotencyKey]
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status409Conflict)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status422UnprocessableEntity)]
    public async Task<IActionResult> Atualizar(Guid id, [FromBody] AtualizarTipoProcessoCommand command, CancellationToken cancellationToken)
    {
    ...
    [HttpDelete("admin/tipos-processo/{id:guid}")]
    [Authorize(Roles = "plataforma-admin")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status403Forbidden)]
